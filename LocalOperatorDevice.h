...
---------------------------------------------------------------------------------------*/
template<class DataInType,class DataOutType,class Oper>
__global__ void G_LocalOperMultiLayersWithNoData(DataInType* input, DataOutType* output, int width, int height, DataInType* nodataIn, DataOutType nodataOut, int numInRaster, Oper op)
{
	int x_idx = blockIdx.x*blockDim.x + threadIdx.x;
	if (x_idx >= width)
		return;
